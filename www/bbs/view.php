	...
	if($setup[use_comment]) {
		while($c_data=mysql_fetch_array($view_comment_result)) {
			...
			if($c_data[ismember]) {
				...
				$c_data[memo]=preg_replace($imageBoxPattern,"<img src='icon/member_image_box/$c_data[ismember]/\\1.\\2' name=zb_target_resize style=\"cursor:pointer\" onclick=\"javascript: var IMG = new Image(); IMG.src = 'icon/member_image_box/$c_data[ismember]/\\1.\\2'; var w = IMG.width; var h = IMG.height; window.open('img_view.php?img=icon/member_image_box/$c_data[ismember]/\\1.\\2&width='+(w+10)+'&height='+(h+55),'imgViewer','width=0,height=0,toolbar=no,scrollbars=no','status=no')\" align='\\3' width='\\4' height='\\5' vspace='\\6' hspace='\\7' border='\\8'>",$c_data[memo]);
			}
			...
			if(preg_match("#\.(jpg|jpeg|png|gif|bmp)$#i",$c_file_name1)) $c_upload_image1="<img src=$c_file_name1_ border=0 width=640 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
			...
			elseif(preg_match("#\.(mp3|mp4|ogg|oga|mov|flv|m4v|f4v|webm|aac|m4a|f4a)$#i",$c_file_name1)) $c_upload_image1="<script src='/bbs/jwplayer/jwplayer.js'></script><div id='jwplayer2'>Loading the player ...</div><script>jwplayer('jwplayer2').setup({flashplayer: '/bbs/jwplayer/player.swf', file: '$c_file_name1_', width: 640, height: 480});</script>";

			if(preg_match("#\.(jpg|jpeg|png|gif|bmp)$#i",$c_file_name2)) $c_upload_image2="<img src=$c_file_name2_ border=0 width=640 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
			...
			elseif(preg_match("#\.(mp3|mp4|ogg|oga|mov|flv|m4v|f4v|webm|aac|m4a|f4a)$#i",$c_file_name2)) $c_upload_image2="<script src='/bbs/jwplayer/jwplayer.js'></script><div id='jwplayer3'>Loading the player ...</div><script>jwplayer('jwplayer3').setup({flashplayer: '/bbs/jwplayer/player.swf', file: '$c_file_name2_', width: 640, height: 480});</script>";
