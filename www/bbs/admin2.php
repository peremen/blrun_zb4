...
$member=member_info();
if((($member[no]&&$member[is_admin]<3&&$member[is_admin]>=1)||($member[no]&&$member[board_name]))&&$_SESSION['_token2']) movepage("admin_setup_bac.php?exec=$exec&no=$no&group_no=$group_no&exec2=$exec2&_token2=".$_SESSION['_token2']);
else {
	if($member[no]) {
...
<input type=hidden name=exec2 value=modify>
<div align=center>
