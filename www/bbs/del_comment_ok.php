...
// 코멘트 삭제
mysqli_query($connect,"delete from $t_comment"."_$id where no='$c_no'") or error(mysqli_error($connect));
...
// 회원일 경우 해당 해원의 점수 주기
if($member[no]==$s_data[ismember]) @mysqli_query($connect,"update $member_table set point2=point2-1 where no='$member[no]'") or error(mysqli_error($connect));
