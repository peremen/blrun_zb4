...
if($file1_size>0&&$setup[use_pds]&&$file1) {
	...
	if($file1_size>0) {
		...
		if(substr($s_file_name1,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name1)) Error("Html, PHP 관련파일은 업로드할수 없습니다");
...
if($file2_size>0&&$setup[use_pds]&&$file2) {
	...
	if($file2_size>0) {
		...
		if(substr($s_file_name2,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name2)) Error("Html, PHP 관련파일은 업로드할수 없습니다");
