		...
		<tr>
			...
			<td width=100% height=100% class=list1>
				...
				<tr>
					...
					<td width=70><input type=button class=submit value='임시저장' onclick=autoSave() accesskey="a" style="height:50%" title="1주일간 글을 임시보관 합니다"><br><input type=submit class=submit value='수정완료' accesskey="s" style="height:50%"></td>
				</tr>
				</table>
			</td>
		</tr>
...
<?
	...
	if($file1_size>0&&$setup[use_pds]&&$file1) {
		...
		if($file1_size>0) {
			...
			if(mb_substr($s_file_name1,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name1)) Error("Html, PHP 관련파일은 업로드할수 없습니다");
			...
			if(file_exists("data/$id/".$s_file_name1)) {
				...
				if(!move_uploaded_file($file1,"data/$id/".$regdate."/".$s_file_name1)) Error("파일업로드가 제대로 되지 않았습니다");
	...
	if($file2_size>0&&$setup[use_pds]&&$file2) {
		...
		if($file2_size>0) {
			...
			if(mb_substr($s_file_name2,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name2)) Error("Html, PHP 관련파일은 업로드할수 없습니다");
			...
			if(file_exists("data/$id/".$s_file_name2)) {
				$regdate=getMicrosecond(); // 마이크로세컨드 시간구함;;
				@mkdir("data/$id/".$regdate,0777);
				if(!move_uploaded_file($file2,"data/$id/".$regdate."/".$s_file_name2)) Error("파일업로드가 제대로 되지 않았습니다");
	...
	if($mode=="modify") {
		$_dbTimeStart = getmicrotime();
		mysqli_query($connect,"delete from $comment_imsi_table where bname='$id' and cno='$c_no' and parent='$no' and ismember='$ismember' and name='$member[name]'");
		$_dbTime += getmicrotime()-$_dbTimeStart;
	}
