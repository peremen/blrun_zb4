...
// 필요한 변수 받아내기
	...
	for( $i = 0; $i < sizeof($id); $i++){
		$setup = mysqli_fetch_array(mysqli_query($connect,"select use_alllist, use_category from $admin_table where name='".$id[$i]."'"));
...
// 최근 글 순으로 게시물 뽑음
	for( $i = 0; $i < sizeof($id); $i++){
		...
		while($data=mysqli_fetch_array($result)){
			...
			$last_comment = mysqli_fetch_array(mysqli_query($connect,"select * from $t_comment"."_$id[$i] where parent='$data[no]' order by reg_date desc limit 1"));
			...
			$result_category = mysqli_query($connect,"select * from zetyx_board_category_".$id[$i]." where no='$category'") or die(mysqli_error($connect));
