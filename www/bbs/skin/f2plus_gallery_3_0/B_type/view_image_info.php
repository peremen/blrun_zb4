...
if($Thumbnail_use=="on" && $Thumbnail_view=="on"){       //썸네일 사용시
	if($upload_image1){
		...
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name1])){
			...
			$view_img1="<img src=$thumb_view1 border=0 id=zb_target_resize alt='$alt1'>";
		}else{
	...
	if($upload_image2){
		...
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name2])){
			...
			$view_img2="<img src=$thumb_view2 border=0 id=zb_target_resize alt='$alt2'>";
		}else{
	...
	if(preg_match("#\.(jpg|jpeg|png)$#i",$prev_data[file_name1])){
		...
		if(!file_exists($Thumbnail_path.$prev_data[ismember]."/".$Prev_thumb_small1)){
			$zx=thumbnail($min_width_size,$src_img1,$Thumbnail_path.$prev_data[ismember]."/".$Prev_thumb_small1);
	...
	if(preg_match("#\.(jpg|jpeg|png)$#i",$next_data[file_name1])){
		...
		if(!file_exists($Thumbnail_path.$next_data[ismember]."/".$Next_thumb_small1)){
			$zx=thumbnail($min_width_size,$src_img2,$Thumbnail_path.$next_data[ismember]."/".$Next_thumb_small1);
	...
	if($upload_image1){
		$source_img=str_replace("%2F", "/", urlencode($data[file_name1]));
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name1])){
			$view_img1="<img src=$source_img id=zb_target_resize border=0 alt='$alt1'>";
		}else{
			$view_img1=str_replace("<br>","",$upload_image1);
			$view_img1=str_replace("cursor:pointer","",$view_img1);
			$view_img1=preg_replace("#onclick=\"javascript\:[^>]+?(>)#i","alt='$alt1'\\1",$view_img1);
	...
	if($upload_image2){
		$source_img=str_replace("%2F", "/", urlencode($data[file_name2]));
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name2])){
			$view_img2="<img src=$source_img id=zb_target_resize border=0 alt='$alt2'>";
		}else{
			$view_img2=str_replace("<br>","",$upload_image2);
			$view_img2=str_replace("cursor:pointer","",$view_img2);
			$view_img2=preg_replace("#onclick=\"javascript\:[^>]+?(>)#i","alt='$alt2'\\1",$view_img2);
