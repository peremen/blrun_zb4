...
if($file1_size>0&&$setup[use_pds]&&$file1) {
	...
	if($file1_size>0) {
		...
		if(substr($s_file_name1,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name1)) Error1("Html, PHP 관련파일은 업로드할수 없습니다");
...
if($file2_size>0&&$setup[use_pds]&&$file2) {
	...
	if($file2_size>0) {
		...
		if(substr($s_file_name2,0,1)=='.'||preg_match("#\.(inc|phtm|htm|shtm|phtml|html|shtml|ztx|php\d{0,}|dot|asp|cgi|pl)$#i",$s_file_name2)) Error1("Html, PHP 관련파일은 업로드할수 없습니다");
...
if($mode=="modify"&&$c_no) {
	...
	mysqli_query($connect,"update $t_comment"."_$id set name='$name',memo='$memo',use_html2='$use_html2',is_secret='$is_secret' $del_que1 $del_que2 where no='$c_no'") or Error1(mysqli_error($connect));
	if($type=="Movie_type"||$type=="Sell_type") mysqli_query($connect,"update $t_comment"."_$id"."_movie set point1='$_point1',point2='$_point2' where parent='$no' and reg_date='$c_date'") or Error1(mysqli_error($connect));

} elseif($mode=="write"||($mode=="reply"&&$c_no)) {
	// 코멘트 입력
	mysqli_query($connect,"insert into $t_comment"."_$id (parent,ismember,islevel,name,password,memo,reg_date,ip,use_html2,is_secret,file_name1,file_name2,s_file_name1,s_file_name2) values ('$no','$member[no]','$member[level]','$name','$password','$memo','$reg_date','$ip','$use_html2','$is_secret','$file_name1','$file_name2','$s_file_name1','$s_file_name2')") or Error1(mysqli_error($connect));
	if($type=="Movie_type"||$type=="Sell_type") mysqli_query($connect,"insert into $t_comment"."_$id"."_movie (parent,reg_date,point1,point2) values ('$no','$reg_date','$_point1','$_point2')") or Error1(mysqli_error($connect));
	// 회원일 경우 해당 해원의 점수 주기
	mysqli_query($connect,"update $member_table set point2=point2+1 where no='$member[no]'") or Error1(mysqli_error($connect));
}
...
mysqli_query($connect,"update $t_board"."_$id set total_comment='$total[0]' where no='$no'") or Error1(mysqli_error($connect));
