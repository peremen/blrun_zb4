...
<tr align=center height=50 style=padding-bottom:3px class=list<?=$coloring%2?>>
	...
	<?if($hide_no!="on"){?>
	<? if($browser=="1"){ ?><td class=list_main2 align=center nowrap><div style="overflow:hidden"><font class=title_font style=color:FF4E00>notice</font></div></td><? } ?><?}?>
	...
	<td class=list_notice3>
		...
		<tr>
			<td align=left style=padding-top:7px nowrap><div style="overflow:hidden"><?=$hide_cart_start?><input type=checkbox name=cart value="<?=$data[no]?>"><?=$hide_cart_end?>&nbsp;&nbsp;<img src=<?=$dir?>/images/notice_fr2.gif align=absmiddle border=0>&nbsp;<?=$insert?><B><?=$subject?></B><?=$comment_new?></td> 
		...
		<tr valign=bottom>
			<td align=left style=padding-top:6px><div style="overflow:hidden"><nobr><font style=color:ffffff>&nbsp;&nbsp;&nbsp;
			<? if(!$data[is_secret]) echo substr(stripslashes($tmp_memo),0,400); else echo "비밀글입니다"; ?>
			</font></nobr></div></td>
			<?if($hide_name!="on"){?><? if($browser=="1"){ ?>
			<td align=center class=com7 style=padding-bottom:2px><div style="overflow:hidden"><img src=<?=$dir?>/images/point.gif align=absmiddle>&nbsp;<?=($m_data[point1]*10+$m_data[point2])?></td><? } ?><?}?>
			<?if($hide_date!="on"){?><? if($browser=="1"){ ?>
			<td align=center style=padding-top:8px><font class=com7 color=aaaaaa><?=date("H:i:s",$data[reg_date])?></font></td><? } ?><?}?>
