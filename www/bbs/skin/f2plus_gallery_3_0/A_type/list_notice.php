...
<tr align=center height=45 style=padding-bottom:0px class=list<?=$coloring%2?>>
	<?if($hide_no=="off"){?>
	<? if($browser=="1"){ ?><td style=padding-top:0px;padding-bottom:4px class=com4 align=center nowrap><div style="overflow:hidden"><img src=<?=$dir?>/images/notice_fr.gif align=absmiddle border=0></div></td><? } ?><?}?>
	...
	<td>
		...
		<tr>
			<td align=left nowrap><div style="overflow:hidden"><?=$hide_cart_start?><input type=checkbox name=cart value="<?=$data[no]?>"><?=$hide_cart_end?><img src=<?=$dir?>/images/notice_fr2.gif align=absmiddle border=0>&nbsp;<?=$insert?><B><?=$subject?></B><?=$comment_new?></font></div></td> 
