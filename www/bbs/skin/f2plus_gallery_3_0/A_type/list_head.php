...
<tr><td>
		<table border=0 cellspacing=0 cellpadding=0 width=100% align=center style=border-width:1pt;border-style:solid;border-color:cccccc style=table-layout:fixed>
			...
			<?if($hide_no=="off"){?><? if($browser=="1"){ ?><col width=40></col><? } ?><?}?>
			<? if($browser=="1"){ ?><col width=80></col><? } ?>
			<col width=></col>
			...
			<?if($hide_no=="off"){?><? if($browser=="1"){ ?><td><?=$a_no?><img src=<?=$dir?>/images/top_no.gif border=0></td><? } ?><?}?>
			<? if($browser=="1"){ ?><td><img src=<?=$dir?>/images/top_img.gif border=0></td><? } ?>
			<td><?=$a_subject?><img src=<?=$dir?>/images/top_subject.gif border=0></td>
			<?if($hide_name=="off"){?><? if($browser=="1"){ ?><td align=center><?=$a_name?><img src=<?=$dir?>/images/top_name.gif border=0></td><? } ?><?}?>
			<?if($hide_date=="off"){?><? if($browser=="1"){ ?><td><?=$a_date?><img src=<?=$dir?>/images/top_date.gif border=0></td><? } ?><?}?>
			<?if($hide_vote=="off"){?><? if($browser=="1"){ ?><td><?=$a_vote?><img src=<?=$dir?>/images/top_vote.gif border=0></td><? } ?><?}?>
			<?if($hide_hit=="off"){?><? if($browser=="1"){ ?><td><?=$a_hit?><img src=<?=$dir?>/images/top_hit.gif border=0></td><? } ?><?}?>	
			</tr>
		</table>
