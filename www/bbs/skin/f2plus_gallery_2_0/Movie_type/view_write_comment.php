...
<td><input type=password id=password name=password <?=size(8)?> maxlength=20 class=input onkeyup="ajaxLoad2()" title="이름과 비번을 재입력하면 임시저장이 복원됨"></td>
<?=$hide_c_password_end?>

<td width="100%" align=left><div align=<?=$align?>><?if ($emoticon_use=="on"){?>
...
<tr><td colspan=5>
	...
	<td align=left>
		...
		<tr>
			...
			<td width=90><input type=button class=submit value=' 임시 저장 ' onclick=autoSave() accesskey="a" style="height:50%" title="1주일간 글을 임시보관 합니다"><br><input type=submit class=submit value='영화평쓰기' accesskey="s" style=height:50%></td>
		</tr>
		</table>
