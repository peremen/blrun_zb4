...
if($emoticon_use=="on") $tmp_memo=str_to_emoticon($tmp_memo,$emoticon_url);
for($i=1;$i<=$_name6;$i++){
	$_point.="★";
}
...
if($counter%$num==1){?>
	...
	<td align=center valign=top>
		...
		<td align=left valign=top><?=$view_img?><img src=<?=$thumb_img?> border=0 align=left width=<?=$min_width_size?> height=<?=$min_width_size*3/2?> class=shadow2 style=border-width:4pt;border-style:solid;border-color:#f1f1f1;></a>
			...
			<?=$hide_cart_start?><input type=checkbox name=cart value="<?=$data[no]?>"><?=$hide_cart_end?><B><?=$hide_category_start?>[<?=$category_name?>] <?=$hide_category_end?><?=$subject?></font></B><BR></font>
			<table width=100% border=0 cellspacing=0 cellpadding=0>
			...
			<tr><td height=1 background=<?=$dir?>/dot.gif></td></tr>
			<tr><td height=4></td></tr></table>
			<img src=<?=$dir?>/point_fr2.gif border=0 align=absmiddle> :&nbsp;<?=$_point?><BR>
