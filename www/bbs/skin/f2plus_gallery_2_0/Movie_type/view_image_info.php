...
if($Thumbnail_use=="on" && $Thumbnail_view=="on"){       //썸네일 사용시
	if($upload_image1){
		...
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name1])){
			...
			$view_img1="<img src=$thumb_view1 border=0 id=zb_target_resize align=left class=shadow5>";
		}else{
	...
	if($upload_image2){
		...
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name2])){
			...
			$view_img2="<img src=$thumb_view2 border=0 id=zb_target_resize align=left class=shadow5>";
		}else{
