...
if($Thumbnail_use=="on" && $Thumbnail_view=="on"){       //썸네일 사용시
	if($upload_image1){
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name1])){
			if(!file_exists($Thumbnail_path.$view_large1)){
				thumbnail($max_width_size,$data[file_name1],$Thumbnail_path.$view_large1);
			}
	...
	if($upload_image2){
		if(preg_match("#\.(jpg|jpeg|png)$#i",$data[file_name2])){
			if(!file_exists($Thumbnail_path.$view_large2)){
				thumbnail($max_width_size,$data[file_name2],$Thumbnail_path.$view_large2);
			}
	...
	if(preg_match("#\.(jpg|jpeg|png)$#i",$prev_data[file_name1])){
		...
		if(!file_exists($Thumbnail_path.$Prev_thumb_small1)){
			thumbnail($min_width_size,$prev_data[file_name1],$Thumbnail_path.$Prev_thumb_small1);
		}
		$prev_thumb=$Thumbnail_url.$Prev_thumb_small1;
		...
		if(!file_exists($Thumbnail_path.$prev_data[ismember]."/".$Prev_thumb_small1)){
			thumbnail($min_width_size,$src_img1,$Thumbnail_path.$prev_data[ismember]."/".$Prev_thumb_small1);
		}
		$prev_thumb=$Thumbnail_url.$prev_data[ismember]."/".$Prev_thumb_small1;

	}elseif(preg_match("#\.(jpg|jpeg|png)$#i",$prev_data[file_name2])){

		if(!file_exists($Thumbnail_path.$Prev_thumb_small2)){
			thumbnail($min_width_size,$prev_data[file_name2],$Thumbnail_path.$Prev_thumb_small2);
		}
		...
		if(file_exists($src_img1) && !file_exists($Thumbnail_path.$prev_data[ismember]."/".$iPrev_thumb_small2)){
			thumbnail($min_width_size,$src_img1,$Thumbnail_path.$prev_data[ismember]."/".$iPrev_thumb_small2);
			$prev_thumb=$Thumbnail_url.$prev_data[ismember]."/".str_replace("%2F", "/", urlencode($iPrev_thumb_small2));
		}elseif(!file_exists($src_img1)){
			$prev_thumb=$dir."/no_image.gif";
		}else{
			$prev_thumb=$Thumbnail_url.$prev_data[ismember]."/".str_replace("%2F", "/", urlencode($iPrev_thumb_small2));
	...
	if(preg_match("#\.(jpg|jpeg|png)$#i",$next_data[file_name1])){
		...
		if(!file_exists($Thumbnail_path.$Next_thumb_small1)){
			thumbnail($min_width_size,$next_data[file_name1],$Thumbnail_path.$Next_thumb_small1);
		}
		...
		if(!file_exists($Thumbnail_path.$next_data[ismember]."/".$Next_thumb_small1)){
			thumbnail($min_width_size,$src_img2,$Thumbnail_path.$next_data[ismember]."/".$Next_thumb_small1);
		}
		$next_thumb=$Thumbnail_url.$next_data[ismember]."/".$Next_thumb_small1;

	}elseif(preg_match("#\.(jpg|jpeg|png)$#i",$next_data[file_name2])){

		if(!file_exists($Thumbnail_path.$Next_thumb_small2)){
			thumbnail($min_width_size,$next_data[file_name2],$Thumbnail_path.$Next_thumb_small2);
		}
		...
		if(!file_exists($Thumbnail_path.$next_data[ismember]."/".$iNext_thumb_small2)){
			thumbnail($min_width_size,$src_img2,$Thumbnail_path.$next_data[ismember]."/".$iNext_thumb_small2);
			$next_thumb=$Thumbnail_url.$next_data[ismember]."/".str_replace("%2F", "/", urlencode($iNext_thumb_small2));
		}elseif(!file_exists($src_img2)){
			$next_thumb=$dir."/no_image.gif";
		}else{
			$next_thumb=$Thumbnail_url.$next_data[ismember]."/".str_replace("%2F", "/", urlencode($iNext_thumb_small2));
