<?
	...
	$number= str_replace("arrow.gif","arrow3.gif",$number);

	if($prev_no==$data[no] && $hide_no=="on") $subject="<img src=$dir/arrow3.gif border=0>&nbsp;".$subject;
...
<tr align=center height=45 style=padding-bottom:0px class=list<?=$coloring%2?>>
	<?if($hide_no=="off"){?>
	<? if($browser=="1"){ ?><td style=padding-top:0px;padding-bottom:4px class=list_eng align=center nowrap><div style="overflow:hidden"><?=$number?></div></td><? } ?><?}?>
	<? if($browser=="1"){ ?><td style=padding-top:0px;padding-bottom:4px border=0 align=center><?=$view_img?><img src=<?=$thumb_img?> width=52 height=39 style=border-width:1pt;border-style:solid;border-color:#000000;></td><? } ?>
	<td>
		<table border=0 cellspacing=0 cellpadding=0 width=100% align=center style=table-layout:fixed>
		<col width=100%></col>
		<?if($hide_name=="off"){?><? if($browser=="1"){ ?><col width=100></col><? } ?><?}?>
		<?if($hide_date=="off"){?><? if($browser=="1"){ ?><col width=65></col><? } ?><?}?>
		<?if($hide_vote=="off"){?><? if($browser=="1"){ ?><col width=35></col><? } ?><?}?>
		<?if($hide_hit=="off"){?><? if($browser=="1"){ ?><col width=50></col><? } ?><?}?>
		<tr>
			...
			<?if($hide_hit=="off"){?><? if($browser=="1"){ ?><td nowrap class=list_eng align=center><?=$hit?></td><? } ?><?}?></tr>
			<tr><td background=<?=$dir?>/dot.gif border=0 height=1 colspan=<?=$cols?>></td>
		</tr>
		<tr valign=middle>
			<td align=left><div style="overflow:hidden"><nobr><font style=color:B0B0B0>
			<? if(!$data[is_secret]) echo substr(stripslashes($_name1[0]),0,400); else echo "비밀글입니다"; ?>
			</font></nobr></div></td>
			<?if($hide_name=="off"){?><? if($browser=="1"){ ?>
			<td align=center class=com3><div style="overflow:hidden"><img src=<?=$dir?>/point.gif align=absmiddle>&nbsp;<?=($m_data[point1]*10+$m_data[point2])?></div></td><? } ?><?}?>
			<?if($hide_date=="off"){?><? if($browser=="1"){ ?>
			<td align=center><font class=com3 color=aaaaaa><?=date("H:i:s",$data[reg_date])?></font></td><? } ?><?}?>
			<?if($hide_vote=="off"){?><? if($browser=="1"){ ?><td></td><? } ?><?}?>
			<?if($hide_hit=="off"){?><? if($browser=="1"){ ?><td align=center valign=bottom>
			<? if ($data[hit]<$max_hit/15)
						...
						elseif ($data[hit]<$max_hit*2/3)
							echo "<img src=$dir/hit_gra3.gif border=0 align=absmiddle>";
						elseif ($data[hit]<$max_hit)
							echo "<img src=$dir/hit_gra4.gif border=0 align=absmiddle>";
						else echo "<img src=$dir/hit_gra5.gif border=0 align=absmiddle>";
			?>			
			</td><? } ?><?}?>
		</tr>
		</table>
