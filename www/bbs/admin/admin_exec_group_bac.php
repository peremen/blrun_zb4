...
if($exec=="add_group_ok") {
	...
	// 중복 이름 검사
	$check=mysqli_fetch_array(mysqli_query($connect,"select count(*) from $group_table where name='$name'"));
	...
	if($icon_name) {
		...
		if($result[0]!=$icon_name) Error("파일명은 한글,영문자,숫자,괄호,공백,+,-,_ 만을 사용할 수 있습니다!");

		if(!preg_match("#\.(jpg|jpeg|png|gif|bmp)$#i",$icon_name)) Error("아이콘은 jpg(jepg)/png/gif/bmp 파일을 올려주세요");
	...
	// DB에 입력
	@mysqli_query($connect,"insert into $group_table (name,is_open,icon,use_join,join_return_url, use_icon, header,footer,header_url,footer_url)
	...
	$group_no=mysqli_insert_id($connect);
...
elseif($exec=="modify_group_ok") {
	...
	// DB에 입력
	@mysqli_query($connect,"update $group_table set
...
elseif($exec=="del_group_ok") {
	...
	$num=mysqli_fetch_array(mysqli_query($connect,"select member_num, board_num from $group_table where no='$group_no'"));

	// 멤버 이동
	if($member_move) {
		@mysqli_query($connect,"update $member_table set group_no='$member_move' where group_no='$group_no'") or Error("회원 이동시에 에러가 발생하였습니다");
		mysqli_query($connect,"update $group_table set member_num=member_num+".$num[member_num]." where no='$member_move'");
	} else {
		$result = mysqli_query($connect,"select no from $member_table where group_no='$group_no'") or Error("회원 이동시에 에러가 발생하였습니다");
	...
	// 게시판이동
	if($board_move) {
		@mysqli_query($connect,"update $admin_table set group_no='$board_move' where group_no='$group_no'") or Error("게시판 이동시에 에러가 발생하였습니다");
		mysqli_query($connect,"update $group_table set board_num=board_num+".$num[board_num]." where no='$board_move'");
	} else {
		$temp=mysqli_query($connect,"select name from $admin_table where group_no='$group_no'");
		...
		@mysqli_query($connect,"delete from $admin_table where group_no='$group_no'");
	}

	// 그룹삭제
	@mysqli_query($connect,"delete from $group_table where no='$group_no'") or Error("그룹삭제시 에러가 발생하였습니다");

	movepage("$PHP_SELF?sid=$sid");
}
// 가입양식 변경
elseif($exec=="modify_member_join_ok")                                                                      {
	mysqli_query($connect,"update $group_table set join_level='$join_level',use_icq='$use_icq',use_aol='$use_aol',use_msn='$use_msn',
	...
	use_birth='$use_birth',use_picture='$use_picture' where no='$group_no'") or error(mysqli_error($connect));
