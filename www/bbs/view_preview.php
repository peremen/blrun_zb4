...
// 원 게시글 정보 읽어오기
if($no&&$mode=="modify") $data=mysqli_fetch_array(mysqli_query($connect,"select * from  $t_board"."_$id  where no='$no'"));
...
if($ismember) {
	...
	$memo=preg_replace_callback($imageBoxPattern2,create_function('$match','return "[img:".str_replace("%2F", "/", urlencode("$match[1].$match[2]")).",";'),$memo);
